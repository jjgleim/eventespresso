...
class EE_Paypal extends Espresso_PaymentGateway {
	...
	public function logErrors($errors) {
		if ($this->logIpn) {
			...
			// Write to log
			file_put_contents($this->ipnLogFile, $text, FILE_APPEND) or do_action('action_hook_espresso_log', __FILE__, __FUNCTION__, 'could not write to paypal log file');
		}
	}
	...
	public function submitButton($button_url, $gateway) {
		...
		foreach ($this->fields as $name => $value) {
			if($name != 'amount_1') {
				...
				echo "<input id='amount_1' name=\"$name\" value=\"$value\"/>\n";
			}
		}
