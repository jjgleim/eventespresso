...
if (!function_exists('event_espresso_get_event_details')) {
	...
	function event_espresso_get_event_details( $attributes ) {
		...
		$raw_events = $wpdb->get_results( $wpdb->prepare($sql, ''));
		$events = array();
		foreach($raw_events as $raw_event) {
			if (isset($events[$raw_event->id])) {
				$events[$raw_event->id]->event_cost[] = $raw_event->event_cost;
				sort($events[$raw_event->id]->event_cost);
			} else {
				$raw_event->event_cost = array($raw_event->event_cost);
